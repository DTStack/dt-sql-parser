...
generic_set: var_name (TO | EQUAL) var_list;
...
altertablestmt:
	...
	| ALTER INDEX (IF_P EXISTS)? qualified_name (
		alter_table_cmds
		| index_partition_cmd
	)
	...
	| ALTER MATERIALIZED VIEW ALL IN_P TABLESPACE name (
		OWNED BY role_list
...
alter_table_cmd:
	...
	| ALTER opt_column? colid ADD_P GENERATED generated_when AS IDENTITY_P
...
copystmt:
	COPY opt_binary? qualified_name opt_column_list? copy_from opt_program? copy_file_name
		copy_delimiter? opt_with? copy_options where_clause?
	| COPY OPEN_PAREN preparablestmt CLOSE_PAREN TO opt_program? copy_file_name opt_with?
		copy_options;
...
createstmt:
	CREATE opttemp? TABLE (IF_P NOT EXISTS)? qualified_name (
		...
		| PARTITION OF qualified_name opttypedtableelementlist? partitionboundspec optpartitionspec?
			table_access_method_clause? optwith? oncommitoption? opttablespace?
	);
...
constraintelem:
	...
	| EXCLUDE access_method_clause? OPEN_PAREN exclusionconstraintlist CLOSE_PAREN opt_c_include?
		opt_definition? optconstablespace? exclusionwhereclause? constraintattributespec
	| FOREIGN KEY OPEN_PAREN columnlist CLOSE_PAREN REFERENCES qualified_name opt_column_list?
...
creatematviewstmt:
	CREATE optnolog? MATERIALIZED VIEW (IF_P NOT EXISTS)? create_mv_target AS selectstmt
		opt_with_data?;
...
createforeignserverstmt:
	CREATE SERVER name opt_type? opt_foreign_server_version? FOREIGN DATA_P WRAPPER name
		create_generic_options?
	| CREATE SERVER IF_P NOT EXISTS name opt_type? opt_foreign_server_version? FOREIGN DATA_P
		WRAPPER name create_generic_options?;
...
createforeigntablestmt:
	...
	| CREATE FOREIGN TABLE IF_P NOT EXISTS qualified_name OPEN_PAREN opttableelementlist?
		CLOSE_PAREN optinherit? SERVER name create_generic_options?
	| CREATE FOREIGN TABLE qualified_name PARTITION OF qualified_name opttypedtableelementlist?
		partitionboundspec SERVER name create_generic_options?
	| CREATE FOREIGN TABLE IF_P NOT EXISTS qualified_name PARTITION OF qualified_name
		opttypedtableelementlist? partitionboundspec SERVER name create_generic_options?;

importforeignschemastmt:
	IMPORT_P FOREIGN SCHEMA name import_qualification? FROM SERVER name INTO name
		create_generic_options?;
...
createpolicystmt:
	CREATE POLICY name ON qualified_name rowsecuritydefaultpermissive? rowsecuritydefaultforcmd?
...
createtrigstmt:
	CREATE TRIGGER name triggeractiontime triggerevents ON qualified_name triggerreferencing?
		triggerforspec? triggerwhen? EXECUTE function_or_procedure func_name OPEN_PAREN
		triggerfuncargs CLOSE_PAREN
	| CREATE CONSTRAINT TRIGGER name AFTER triggerevents ON qualified_name optconstrfromtable?
		constraintattributespec FOR EACH ROW triggerwhen? EXECUTE function_or_procedure func_name
...
createeventtrigstmt:
	CREATE EVENT TRIGGER name ON collabel EXECUTE function_or_procedure func_name OPEN_PAREN
		CLOSE_PAREN
	| CREATE EVENT TRIGGER name ON collabel WHEN event_trigger_when_list EXECUTE
...
createopclassstmt:
	CREATE OPERATOR CLASS any_name opt_default? FOR TYPE_P typename USING name opt_opfamily? AS
		opclass_item_list;
...
object_type_name_on_any_name: POLICY | RULE | TRIGGER;
...
defaclaction:
	...
	| REVOKE GRANT OPTION FOR privileges ON defacl_privilege_target FROM grantee_list
		opt_drop_behavior?;
...
indexstmt:
	CREATE opt_unique? INDEX opt_concurrently? opt_index_name? ON relation_expr access_method_clause
		? OPEN_PAREN index_params CLOSE_PAREN opt_include? opt_reloptions? opttablespace?
		where_clause?
	| CREATE opt_unique? INDEX opt_concurrently? IF_P NOT EXISTS name ON relation_expr
...
opt_nulls_order: NULLS_P FIRST_P | NULLS_P LAST_P;

createfunctionstmt:
	CREATE opt_or_replace? (FUNCTION | PROCEDURE) func_name func_args_with_defaults (
		RETURNS (
			func_return
			| TABLE OPEN_PAREN table_func_column_list CLOSE_PAREN
		)
	)? createfunc_opt_list;
...
createcaststmt:
	CREATE CAST OPEN_PAREN typename AS typename CLOSE_PAREN WITH FUNCTION function_with_argtypes
		cast_context?
...
createtransformstmt:
	CREATE opt_or_replace? TRANSFORM FOR typename LANGUAGE name OPEN_PAREN transform_element_list
		CLOSE_PAREN;

transform_element_list:
	FROM SQL_P WITH FUNCTION function_with_argtypes COMMA TO SQL_P WITH FUNCTION
		function_with_argtypes
	| TO SQL_P WITH FUNCTION function_with_argtypes COMMA FROM SQL_P WITH FUNCTION
		function_with_argtypes
...
reindexstmt:
	...
	| REINDEX OPEN_PAREN reindex_option_list CLOSE_PAREN reindex_target_type opt_concurrently?
		qualified_name
	| REINDEX OPEN_PAREN reindex_option_list CLOSE_PAREN reindex_target_multitable opt_concurrently?
		name;
...
rulestmt:
	CREATE opt_or_replace? RULE name AS ON event TO qualified_name where_clause? DO opt_instead?
		ruleactionlist;
...
unlistenstmt: UNLISTEN colid | UNLISTEN STAR;
...
opt_check_option: WITH (CASCADED | LOCAL)? CHECK OPTION;
...
altertsconfigurationstmt:
	...
	| ALTER TEXT_P SEARCH CONFIGURATION any_name ALTER MAPPING FOR name_list REPLACE any_name
		any_with any_name
...
executestmt:
	...
	| CREATE opttemp? TABLE IF_P NOT EXISTS create_as_target AS EXECUTE name execute_param_clause?
		opt_with_data?;
...
insert_rest:
	...
	| OPEN_PAREN insert_column_list CLOSE_PAREN (
		OVERRIDING override_kind VALUE_P
	)? selectstmt
	| DEFAULT VALUES;
...
deletestmt:
	opt_with_clause? DELETE_P FROM relation_expr_opt_alias using_clause? where_or_current_clause?
		returning_clause?;
...
opt_nowait_or_skip: NOWAIT | SKIP_P LOCKED;

updatestmt:
	opt_with_clause? UPDATE relation_expr_opt_alias SET set_clause_list from_clause?
...
into_clause: INTO (opt_strict opttempTableName | into_target);

opt_strict: | STRICT_P;

opttempTableName: (LOCAL | GLOBAL)? (TEMPORARY | TEMP) opt_table? qualified_name
...
constdatetime: (TIMESTAMP | TIME) (OPEN_PAREN iconst CLOSE_PAREN)? opt_timezone?;
...
/*
 ...
 TYPECAST typename -- 1 | a_expr COLLATE any_name -- 2 | a_expr AT TIME ZONE a_expr-- 3 //right
 ...
 FALSE, IS NULL, IS DISTINCT FROM, etc | a_expr IS NOT? ( NULL_P |TRUE_P |FALSE_P |UNKNOWN |DISTINCT
 FROM a_expr |OF OPEN_PAREN type_list CLOSE_PAREN |DOCUMENT_P |unicode_normal_form? NORMALIZED ) --
 ...
 CLOSE_PAREN) -- 21 | UNIQUE select_with_parens -- 22 | DEFAULT -- 23 ;
 */
...
a_expr_or: a_expr_and (OR a_expr_and)*;
/*16*/

a_expr_and: a_expr_in (AND a_expr_in)*;
/*20*/

a_expr_in: a_expr_unary_not (NOT? IN_P in_expr)?;
/*15*/
...
a_expr_like:
	a_expr_qual_op (
		NOT? (LIKE | ILIKE | SIMILAR TO | BETWEEN SYMMETRIC?) a_expr_qual_op opt_escape?
	)?;
/* 8*/
...
func_expr_common_subexpr:
	...
	| XMLELEMENT OPEN_PAREN NAME_P collabel (
...
xml_passing_mech: BY (REF | VALUE_P);
...
decl_stmt: decl_statement | DECLARE | label_decl;
...
opt_scrollable: | NO SCROLL | SCROLL;
...
decl_collate: | COLLATE any_name;

decl_notnull: | NOT NULL_P;

decl_defval: | decl_defkey sql_expression;

decl_defkey: assign_operator | DEFAULT;
...
getdiag_area_opt: | CURRENT_P | STACKED;
...
opt_for_using_expression: | USING expr_list;
...
opt_execute_into: | INTO STRICT_P? into_target;
...
stmt_open:
	OPEN (
		cursor_variable opt_scroll_option FOR (
			selectstmt
			| EXECUTE sql_expression opt_open_using
		)
...
opt_open_using: | USING expr_list;

opt_scroll_option: | opt_scroll_option_no SCROLL;

opt_scroll_option_no: | NO;
//https://www.postgresql.org/docs/current/plpgsql-cursors.html#PLPGSQL-CURSOR-OPENING
...
plsql_opt_transaction_chain: AND NO? CHAIN;
...
exception_sect: | EXCEPTION proc_exceptions;
...
proc_condition: any_identifier | SQLSTATE sconst;
//expr_until_semi:
...
opt_returning_clause_into: INTO opt_strict into_target;
