...
describe('Basic SQL Listener Tests', () => {
    const expectTableName = 'user1';
    const sql = `select id,name,sex from ${expectTableName};`;
    const basic = new BasicSQL();

    const parseTree = basic.parse(sql);

    test('Listener exitTableName', () => {
        class MyListener extends BasicSqlParserListener {
            result = '';
        ...
        basic.listen(listener, parseTree);
    ...
    test('Split sql listener', async () => {
        ...
        const sqlSlices = basic.splitSQLByStatement(sql);
