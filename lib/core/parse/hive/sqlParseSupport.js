...
const stringDistance_1 = require("../stringDistance");
const identifierEquals = (a, b) => a &&
    b &&
    a
        .replace(/^\s*`/, '')
        .replace(/`\s*$/, '')
...
const initSqlParser = function (parser) {
    ...
    parser.getValueExpressionKeywords = function (valueExpression, extras) {
        ...
        if (typeof sqlFunctions_1.SqlFunctions === 'undefined' ||
            sqlFunctions_1.SqlFunctions.matchesType(parser.yy.activeDialect, ['BOOLEAN'], types)) {
            keywords = keywords.concat(['AND', 'OR']);
        }
        if (typeof sqlFunctions_1.SqlFunctions === 'undefined' ||
            sqlFunctions_1.SqlFunctions.matchesType(parser.yy.activeDialect, ['NUMBER'], types)) {
            keywords = keywords.concat(['+', '-', '*', '/', '%', 'DIV']);
        }
        if (typeof sqlFunctions_1.SqlFunctions === 'undefined' ||
            sqlFunctions_1.SqlFunctions.matchesType(parser.yy.activeDialect, ['STRING'], types)) {
    ...
    parser.selectListNoTableSuggest = function (selectListEdit, hasDistinctOrAll) {
        ...
        if (selectListEdit.suggestAggregateFunctions &&
    ...
    parser.findReturnTypes = function (functionName) {
        return typeof sqlFunctions_1.SqlFunctions === 'undefined'
            ? ['T']
            : sqlFunctions_1.SqlFunctions.getReturnTypes(parser.yy.activeDialect, functionName.toLowerCase());
    };
    parser.applyArgumentTypesToSuggestions = function (functionName, position) {
        const foundArguments = typeof sqlFunctions_1.SqlFunctions === 'undefined'
            ? ['T']
            : sqlFunctions_1.SqlFunctions.getArgumentTypes(parser.yy.activeDialect, functionName.toLowerCase(), position);
    ...
    parser.commitLocations = function () {
        ...
        while (i--) {
                    ...
                    if (!found.length && location.firstInChain) {
                        found = tablePrimaries.filter(primary => {
                            return (!primary.alias &&
                                primary.identifierChain &&
                                equalIgnoreCase(primary.identifierChain[primary.identifierChain.length - 1].name, location.identifierChain[0].name));
                        });
                    }
                    if (found.length) {
                        if (found[0].identifierChain.length > 1 &&
                            location.identifierChain.length === 1 &&
                            equalIgnoreCase(found[0].identifierChain[0].name, location.identifierChain[0].name)) {
                            location.type = 'database';
                        }
                        else if (found[0].alias &&
                            ...
                            equalIgnoreCase(location.identifierChain[0].name, found[0].identifierChain[found[0].identifierChain.length - 1].name) &&
                            location.identifierChain.length > 1) {
                            location.type = 'column';
                            parser.expandIdentifierChain({
                                tablePrimaries: tablePrimaries,
            ...
            if (location.type === 'column') {
                ...
                else if (location.identifierChain.length === 0 &&
    ...
    const prioritizeSuggestions = function () {
        ...
        SIMPLE_TABLE_REF_SUGGESTIONS.forEach(suggestionType => {
            if (suggestionType !== 'suggestAggregateFunctions' &&
            ...
            else if (typeof parser.yy.result[suggestionType] !== 'undefined' &&
        ...
        if (typeof parser.yy.result.colRef !== 'undefined') {
            if (!parser.yy.result.colRef.linked ||
        ...
        if (typeof parser.yy.result.suggestJoinConditions !== 'undefined') {
            if (typeof parser.yy.result.suggestJoinConditions.tables === 'undefined' ||
                parser.yy.result.suggestJoinConditions.tables.length === 0) {
                delete parser.yy.result.suggestJoinConditions;
            }
        }
        if (typeof parser.yy.result.suggestTables !== 'undefined' &&
    ...
    parser.expandLateralViews = function (lateralViews, originalIdentifierChain, columnSuggestion) {
        ...
        if (typeof lateralViews !== 'undefined') {
            lateralViews
                ...
                if (equalIgnoreCase(firstIdentifier.name, lateralView.tableAlias) &&
                ...
                else if (equalIgnoreCase(firstIdentifier.name, lateralView.tableAlias) &&
                    identifierChain.length === 1 &&
    ...
    parser.expandIdentifierChain = function (options) {
        ...
        if (identifierChain.length > 0 && identifierChain[identifierChain.length - 1].asterisk) {
            ...
            tablePrimaries.forEach(tablePrimary => {
                if (identifierChain.length > 1 && !tablePrimary.subQueryAlias) {
                    ...
                    else if (identifierChain.length === 3 &&
        ...
        if (identifierChain.length > 0) {
            for (let i = 0; i < tablePrimaries.length; i++) {
                ...
                else if (tablePrimaries[i].identifierChain.length > 1 &&
                    identifierChain.length > 1 &&
    ...
    const convertTablePrimariesToSuggestions = function (tablePrimaries) {
        ...
        tablePrimaries.forEach(tablePrimary => {
            if (tablePrimary.identifierChain && tablePrimary.identifierChain.length > 0) {
                ...
                if (tablePrimary.alias) {
                    ...
                    const lastIdentifier = tablePrimary.identifierChain[tablePrimary.identifierChain.length - 1];
        ...
        if (parser.yy.result.suggestColumns.identifierChain &&
            parser.yy.result.suggestColumns.identifierChain.length === 0) {
    ...
    const linkTablePrimaries = function () {
        ...
        if (typeof parser.yy.result.suggestColumns !== 'undefined' &&
            ...
            const tablePrimaries = filterTablePrimariesForOwner(parser.yy.latestTablePrimaries, parser.yy.result.suggestColumns.owner);
    ...
    parser.suggestAggregateFunctions = function () {
        ...
        parser.yy.latestTablePrimaries.forEach(primary => {
            ...
            if (typeof primary.identifierChain !== 'undefined' &&
    ...
    const adjustLocationForCursor = function (location) {
        ...
        if (parser.yy.cursorFound) {
            if (parser.yy.cursorFound.first_line === newLocation.first_line &&
            ...
            if (parser.yy.partialLengths.left === 0 && parser.yy.partialLengths.right === 0) {
                ...
                location = {
                    ...
                    location: {
                        ...
                        last_column: locationIfPresent.last_column -
                            1 +
    ...
    parser.addColumnLocation = function (location, identifierChain) {
        const isVariable = identifierChain.length && /\${[^}]*}/.test(identifierChain[identifierChain.length - 1].name);
        if (isVariable) {
            parser.yy.locations.push({
                type: 'variable',
                location: adjustLocationForCursor(location),
    ...
    parser.parseSql = function (beforeCursor, afterCursor, debug) {
        ...
        if (beforeCursor.length > REASONABLE_SURROUNDING_LENGTH) {
            ...
            const lastReasonableChunk = beforeCursor.substring(beforeCursor.length - REASONABLE_SURROUNDING_LENGTH);
        ...
        try {
            ...
            result = parser.parse(beforeCursor +
...
const initSyntaxParser = function (parser) {
    ...
    parser.parseSyntax = function (beforeCursor, afterCursor, debug) {
        ...
        if (parser.yy.error &&
            ...
            if (parser.yy.error.expected.length === 2 &&
                ...
                if (expected === 'REGULAR_IDENTIFIER') {
                    ...
                    if (/^<[a-z]+>/.test(parser.yy.error.token)) {
                        ...
                        weightedExpected.push({
                            text: text,
                            distance: stringDistance_1.default(parser.yy.error.text, text, true)
                        });
