import { BasicSQL, BasicSqlSplitListener } from 'src/parser/basic';
...
describe('BasicSQL ErrorStrategy test', () => {
    const basicSQL = new BasicSQL();

    // TODO: handle unexpected case
    // test('begin inValid', () => {
    //     const sql = [inValidSQL, validSQL1, validSQL2].join('\n');
    //     // parse with empty errorListener
    //     const parseTree = basicSQL.parse(sql, () => {});
    //     const splitListener = new BasicSqlSplitListener();
    //     basicSQL.listen(splitListener as BasicSqlParserListener, parseTree);
