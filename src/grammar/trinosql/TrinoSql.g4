...
statement:
	...
	| KW_CREATE KW_SCHEMA (KW_IF KW_NOT KW_EXISTS)? qualifiedName (
		KW_AUTHORIZATION principal
	)? (KW_WITH properties)?											# createSchema
	...
	| KW_CREATE KW_TABLE (KW_IF KW_NOT KW_EXISTS)? qualifiedName columnAliases? (
		KW_COMMENT string
	)? (KW_WITH properties)? KW_AS (query | '(' query ')') (
		KW_WITH (KW_NO)? KW_DATA
	)? # createTableAsSelect
	| KW_CREATE KW_TABLE (KW_IF KW_NOT KW_EXISTS)? qualifiedName '(' tableElement (
		',' tableElement
	)* ')' (KW_COMMENT string)? (KW_WITH properties)?										# createTable
	| KW_DROP KW_TABLE (KW_IF KW_EXISTS)? qualifiedName											# dropTable
	| KW_INSERT KW_INTO qualifiedName columnAliases? query								# insertInto
	...
	| KW_ALTER KW_TABLE (KW_IF KW_EXISTS)? tableName = qualifiedName KW_RENAME KW_COLUMN (
		KW_IF KW_EXISTS
	)? from = identifier KW_TO to = identifier # renameColumn
	| KW_ALTER KW_TABLE (KW_IF KW_EXISTS)? tableName = qualifiedName KW_DROP KW_COLUMN (
		KW_IF KW_EXISTS
	...
	| KW_ALTER KW_MATERIALIZED KW_VIEW qualifiedName 
		KW_SET KW_PROPERTIES propertyAssignments # setMaterializedViewProperties
	...
	| KW_CREATE KW_ROLE name = identifier 
		(KW_WITH KW_ADMIN grantor)? 
		(KW_IN catalog = identifier)?										# createRole 
	| KW_DROP KW_ROLE name = identifier										# dropRole
	| KW_GRANT roles KW_TO principal 
	(',' principal)*
	(KW_WITH KW_ADMIN KW_OPTION)? 
	(KW_GRANTED KW_BY grantor)? 
	(KW_IN catalog = identifier)?                                          # grantRoles
	| KW_REVOKE (KW_ADMIN KW_OPTION KW_FOR)? roles KW_FROM principal 
	(',' principal)* 
	(KW_GRANTED KW_BY grantor)?
	(KW_IN catalog = identifier)? # revokeRoles
	| KW_SET KW_ROLE (KW_ALL | KW_NONE | role = identifier) (
		KW_IN catalog = identifier
	)? # setRole | KW_GRANT (privilege (',' privilege)* | KW_ALL KW_PRIVILEGES) KW_ON (
		KW_SCHEMA
		| KW_TABLE
	)? qualifiedName KW_TO grantee = principal (KW_WITH KW_GRANT KW_OPTION)? # grant
	| KW_DENY (privilege (',' privilege)* | KW_ALL KW_PRIVILEGES) KW_ON (
		KW_SCHEMA
		| KW_TABLE
	)? qualifiedName KW_TO grantee = principal # deny
	| KW_REVOKE (KW_GRANT KW_OPTION KW_FOR)? (
		privilege (',' privilege)*
		| KW_ALL KW_PRIVILEGES
	) KW_ON (KW_SCHEMA | KW_TABLE)? qualifiedName KW_FROM grantee = principal	# revoke
	| KW_SHOW KW_GRANTS (KW_ON KW_TABLE? qualifiedName)?						# showGrants
	| KW_EXPLAIN KW_ANALYZE? KW_VERBOSE? (
	...
	| KW_UPDATE qualifiedName KW_SET updateAssignment (
		',' updateAssignment
	...
	| KW_SHOW KW_COMMENT KW_ON KW_TABLE qualifiedName													# showTableComment
	| KW_SHOW KW_COMMENT KW_ON KW_COLUMN qualifiedName													# showColumnComment;

query: with? queryNoWith;

with: KW_WITH KW_RECURSIVE? namedQuery (',' namedQuery)*;
...
likeClause:
	KW_LIKE qualifiedName (
		optionType = (KW_INCLUDING | KW_EXCLUDING) KW_PROPERTIES
	)?;
...
queryNoWith:
	queryTerm (KW_ORDER KW_BY sortItem (',' sortItem)*)? (
		...
		| (
			KW_FETCH (KW_FIRST | KW_NEXT) (fetchFirst = rowCount)? (
				KW_ROW
				| KW_ROWS
...
querySpecification:
	KW_SELECT setQuantifier? selectItem (',' selectItem)* (
...
windowSpecification: (existingWindowName = identifier)? (
		KW_PARTITION KW_BY partition += expression (
			',' partition += expression
		)*
	)? (KW_ORDER KW_BY sortItem (',' sortItem)*)? windowFrame?;

namedQuery: name = identifier (columnAliases)? KW_AS '(' query ')';
...
joinType: KW_INNER? | KW_LEFT KW_OUTER? | KW_RIGHT KW_OUTER? | KW_FULL KW_OUTER?;
...
sampleType: KW_BERNOULLI | KW_SYSTEM;
...
measureDefinition: expression KW_AS identifier;
...
variableDefinition: identifier KW_AS expression;
...
relationPrimary:
	...
	| KW_UNNEST '(' expression (',' expression)* ')' (
...
predicate[ParserRuleContext value]:
	...
	| KW_NOT? KW_LIKE pattern = valueExpression (
...
primaryExpression:
	...
	)? (KW_ORDER KW_BY sortItem (',' sortItem)*)? ')' filter? (
		nullTreatment? over
	)?															# functionCall
	...
	| KW_CASE operand = expression whenClause+ (
	...
	| KW_SUBSTRING '(' valueExpression KW_FROM valueExpression (
		KW_FOR valueExpression
	)? ')'														# substring
...
processingMode: KW_RUNNING | KW_FINAL;

nullTreatment: KW_IGNORE KW_NULLS | KW_RESPECT KW_NULLS;
...
comparisonQuantifier: KW_ALL | KW_SOME | KW_ANY;

booleanValue: KW_TRUE | KW_FALSE;

interval:
	KW_INTERVAL sign = (PLUS | MINUS)? string from = intervalField (
		KW_TO to = intervalField
	)?;

intervalField: KW_YEAR | KW_MONTH | KW_DAY | KW_HOUR | KW_MINUTE | KW_SECOND;

normalForm: KW_NFD | KW_NFC | KW_NFKD | KW_NFKC;

type:
	KW_ROW '(' rowField (',' rowField)* ')'						# rowType
	| KW_INTERVAL from = intervalField (KW_TO to = intervalField)?	# intervalType
	| base = KW_TIMESTAMP ('(' precision = typeParameter ')')? (
		KW_WITHOUT KW_TIME KW_ZONE
	)?																		# dateTimeType
	| base = KW_TIMESTAMP ('(' precision = typeParameter ')')? KW_WITH KW_TIME KW_ZONE	# dateTimeType
	| base = KW_TIME ('(' precision = typeParameter ')')? (
		KW_WITHOUT KW_TIME KW_ZONE
	)?																	# dateTimeType
...
filter: KW_FILTER '(' KW_WHERE booleanExpression ')';

mergeCase:
	KW_WHEN KW_MATCHED (KW_AND condition = expression)? KW_THEN KW_UPDATE KW_SET targets += identifier EQ values +=
		expression (
...
windowFrame: (
		KW_MEASURES measureDefinition (',' measureDefinition)*
	)? frameExtent (KW_AFTER KW_MATCH skipTo)? (KW_INITIAL | KW_SEEK)? (
		KW_PATTERN '(' rowPattern ')'
	)? (KW_SUBSET subsetDefinition (',' subsetDefinition)*)? (
		KW_DEFINE variableDefinition (',' variableDefinition)*
	)?;
...
privilege: KW_SELECT | KW_DELETE | KW_INSERT | KW_UPDATE;
