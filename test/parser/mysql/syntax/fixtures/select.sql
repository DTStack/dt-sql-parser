...
-- Window frame (frameClause with inlined frameUnits/frameExtent/frameBetween)
...
SELECT id, SUM(val) OVER (PARTITION BY dept ORDER BY ts ROWS BETWEEN 2 PRECEDING AND CURRENT ROW) FROM t1;
SELECT id, SUM(val) OVER (w ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) FROM t1 WINDOW w AS (ORDER BY ts);